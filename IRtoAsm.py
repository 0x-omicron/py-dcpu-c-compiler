...
# @ToDo - when can we use them if ever?
PRT = 'PRT'
PTC = 'PTC'

# Make the reverse lookup table
instrLst = [SET, ADD, SUB, MUL, DIV, MOD, SHL, SHR,
...
def tobytes(instr):
    ...
    if type(instrToByte[instr[0]]) == type(tuple()):
        return [(instrToByte[instr[0]][1] << 4) | (instr[1] << 10)] +\
                list(instr[2:])

    else:
        return [instrToByte[instr[0]] | (instr[1] << 4) | (instr[2] << 10)] +\
                list(instr[3:])
...
def toasm(prog, args=[], globFuncs={}, globVars={}, relativepos=0, DEBUG=False):
    ...
    for k, v in sorted(occurances. items(), key=lambda x: x[1]):
        if len(activeRegisters) >= 4:
            break

        elif((numcalls * 2 + (2 - v)) < 0):
            # It takes 2 words to restore for each function call, as well as
            # 2 to set up. So only do it if we'll get a benefit.
            if k in idents:
    ...
    if DEBUG:
        print "Active registers for this function are", activeRegisters
    ...
    def add(op, a, b, asisA=False, asisB=False, extra=None):
        ...
        for isita, each in track:
            if isita:
                heapalloc.append(
                    (len(sum(map(tobytes, asm[:-1]), [])) + 1, "INTEGER", each))
            else:
                heapalloc.append(
                    (len(sum(map(tobytes, asm), [])) - 1, "INTEGER", each))
    ...
    for instr in prog:
        if instr[0] == IR + LIT:
            ...
            heapalloc.append((len(sum(map(tobytes, asm), [])) - 1,
                              STRING, instr[1]))
            ...
            if instr[3]:
                ...
                add(IFG, instr[1], instr[2])
                          ...
                          IR + REMAINDER, IR + SHIFTL, IR + SHIFTR,
                          IR + BOOLAND, IR + BOOLOR, IR + BOOLXOR]:
            ...
            if instr[1] in globVars:
                ...
                heapalloc.append(
                    (len(sum(map(tobytes, asm), [])) - 1, "INTEGER", instr[1]))
            else:
    ...
    for pos, what in fixgoto:
        asm[pos] = (asm[pos][0], asm[pos][1], asm[pos][2],
                    labelpos[what] + relativepos)
